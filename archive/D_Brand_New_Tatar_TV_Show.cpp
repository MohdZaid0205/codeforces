...
std::vector<std::vector<long long int>> chainable;
chainable.push_back(std::vector<long long int>());
...
for (long long int i=0; i<n; i++){
    ...
    chainable.back().push_back(arr[i]);
    prev = arr[i];
    // lcount ++;
}
bool wins = false;
for (auto part : chainable) {
    long long int uni = 1;
