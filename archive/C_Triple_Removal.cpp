...
// I had to look up the tutorial and this shit is easy as F

std::vector<long long int> zrr;
std::vector<long long int> orr;

zrr.push_back(arr[0] == 0? 1:0);
orr.push_back(arr[0] == 1? 1:0);

for (long long int i = 1; i < n; i++){
    zrr.push_back(zrr.back() + (arr[i] == 0? 1: 0));
    orr.push_back(orr.back() + (arr[i] == 1? 1: 0));
}

std::vector<long long int> adjrr(n, 0);
for (long long int i = 1; i < n; i++) {
    adjrr[i] = adjrr[i - 1] + (arr[i] == arr[i - 1] ? 1 : 0);
}
...
while (q--) {
    ...
    long long int z = zrr[r] - (l > 0 ? zrr[l - 1] : 0);
    long long int o = orr[r] - (l > 0 ? orr[l - 1] : 0);

    if (z % 3 != 0 || o % 3 != 0) {
        println(-1);
    } else {
        long long int len = r - l + 1;
        long long int adj = adjrr[r] - adjrr[l];
        
        println(len/3 + (adj > 0 ? 0: 1));
    }
}
