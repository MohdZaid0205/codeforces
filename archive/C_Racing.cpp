...
std::vector<std::pair<long long int, long long int>> prr(n);
...
long long int min = prr[n-1].second;
// long long int max = INT64_MIN;
for (long long int i = n-2; i >= 0; i--) {
    min = vmin(min, prr[i].second);
    // max = vmax(max, prr[n-i-1].first);
    if (arr[i + 1] == 1 && min == prr[i+1].second)
        min -= 1;
    prr[i].second = min;
    // prr[n-i-1].first = max;
}
