...
typedef struct PrecomputedData {
    long long int temp[64];
};

constexpr PrecomputedData generate() {
    PrecomputedData data = {0};
    data.temp[0] = 1;
    for (int i = 1; i < 60; i++) {
        data.temp[i] = 2 * data.temp[i - 1] + 1;
        // data.temp[i] %= MOD;
    }
    return data;
}
...
// ------------------------------------------------------------------+
    ...
    constexpr auto precomputed = generate();
