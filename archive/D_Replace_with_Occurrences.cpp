...
std::vector<long long int> brr = std::vector<long long int>(n, 0);
for (long long int i = 0; i < n; i++){ std::cin >> arr[i]; }

std::unordered_map<long long int, std::vector<long long int>> pos
= std::unordered_map<long long int, std::vector<long long int>>();
...
for (long long int i = 0; i < n; i++){
    ...
    if (!pos[arr[i]].empty()){
        // println("p", x,":", pos[arr[i]].back(), arr[i]);
        brr[pos[arr[i]].back()] = x;
        pos[arr[i]].pop_back();
    } else {
        possible = false;
    }
