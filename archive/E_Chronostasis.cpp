...
long long int p_sum = 0LL;
long long int n_sum = 0LL;
...
long long int z = 0LL;
std::multiset<long long int> neg = {};
...
long long int i = 0;
// long long int j = static_cast<long long int>(neg.size()) - 1;

long long int curr = 0;
std::vector<long long int> res;
// res.push_back(curr);

while (i < pos.size() && !neg.empty()) {
    auto e = neg.lower_bound(1-curr);

    if (e != neg.end() && curr + *e > 0) {
        curr += *e;
        neg.erase(e);
    } else {
        curr += pos[i];
...
for (long long int l = i; l < pos.size(); l++){
    curr += pos[l];
    res.push_back(curr);
}

std::deque<long long int> end;
for (auto& v:neg){
    end.push_back(v);
}
