...
std::unordered_map<long long int, std::vector<long long int>> connections;
long long int p;
// std::cin >> p;  // parent of 1 is always itself

for (long long int i = 0; i < n - 1; i ++) {
    std::cin >> p;
    connections[p].push_back(i + 2);
}

long long int result = 1;
std::deque<long long int> prev;
std::deque<long long int> next;
prev.push_back(1);

// for (auto parent:connections) {
//     std::cout << "P: " << parent.first << " C: ";
...
std::vector<long long int> dp = std::vector<long long int>(n + 1, 0LL);
long long int level = 0;
dp[1] = 1;
next_generation:
    for (auto& parent:prev) {
        auto child = connections[parent];
        for (auto& each:child) {
            next.push_back(each);
            if (parent != 1) {
                dp[each] -= dp[parent];
...
prev_generation:
    // for (auto& parent:prev) {
    //     result += next.size() - connections[parent].size();
    // }

    for (auto each:next) {
        dp[each] = (dp[each] + level) % mod;
        result = (result + dp[each]) % mod;
        // println("?", each, ">", dp[each]);
    }
...
if (prev.size() > 0) goto next_generation;
