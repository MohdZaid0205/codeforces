...
// double recursive(std::vector<int>& crr, std::vector<int>& prr, int i, double W);
...
std::vector<int> crr = std::vector<int>(n);
std::vector<long double> prr = std::vector<long double>(n);
...
for (long long int i = n-1; i >-1; i--){
    // println(ans, ans*(1-prr[i]/100.0),  crr[i]);
    ans = vmax(ans, ans*(1.0L-prr[i]) + crr[i]);
}
...
//     if (!(i < crr.size()) || W < 0)
//         return 0.0;
//     double choose = recursive( crr, prr, i+1, W*(100-prr[i])/100) + W*crr[i];
//     double ignore = 
// }