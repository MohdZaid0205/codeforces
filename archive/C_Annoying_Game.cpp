...
std::vector<long long int> pre(n, 0);
std::vector<long long int> pst(n, 0);
long long int pre_n = 0LL;
long long int pst_n = 0LL;
...
for (long long int i = 0; i < n; i += 1) {
    ...
    pre_n += arr[i];
    pst_n += arr[n-i-1];
    pre_n = vmax(pre_n, arr[i]);
    pst_n = vmax(pst_n, arr[n-i-1]);
...
if(k%2 == 0){
    long long ans = *std::max_element(pre.begin(),pre.end());
    println(ans);
}
else{
    long long ans = INT64_MIN;
    for(int i = 0;i < n;i++){
        ans = vmax(ans,pre[i]+pst[i]-arr[i]+brr[i]);
    }
    println(ans);
}
