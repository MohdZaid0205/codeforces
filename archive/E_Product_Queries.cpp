...
std::vector<int> arr = std::vector<int>(n, 0);
for (int i = 0; i < n; i++){
    int x; 
    std::cin >> x;
    arr[x - 1] = 1; 
...
for (int i = 1; i < n + 1; i++){
    if (arr[i-1] == 0){
        int prod = INT32_MAX;
        for (int j = 1; j < std::sqrt(i) + 1; j++){
            if (i%j == 0){
                if (arr[j-1] == 0 || arr[i/j - 1] == 0){
                    continue;
                }
                if (arr[j-1]+arr[i/j-1] < prod){
                    prod = arr[j-1]+arr[i/j-1];
                    arr[i-1] = arr[j-1]+arr[i/j-1];
                }
                // println(i, ":", j, i/j, arr[j-1], arr[i/j-1], prod);
            }
        }
    }
}
