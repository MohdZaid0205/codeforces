...
std::vector<long long int> freq = std::vector<long long int>(11, 0);
for (long long int i = 0; i < n; i++){ freq[S[i] - 48] += 1; }

long long int k = vmin(n/4, freq[0]+freq[1]/2);
long long int u0 = k < freq[0] ? k : freq[0];
long long int u1 = k < freq[0] ? 0 : (k - freq[0])*2;
freq[0] -= u0;
freq[1] -= u1;

// println("remaining 1:", freq[1], "remaining 0:", freq[0]);
long long int l = 0;
...
if (l < k){
    long long int r = ((k - l)*2)/3;
    r = vmin(u1, r);
    long long int p = vmin(k - (l + r), u0)/2;
    k = l + r + p;
}
println(k);
// ------------------------------------------------------------------+
