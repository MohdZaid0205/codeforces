...
for (long long int j = 0; j < t; j++){
    ...
    // println("SOLVING:", l, r, k);

    // locate target
    long long int ik = lookup[k];
    // for (long long int i = 0; i < n; i++){
    //     if (arr[i] == k){
    //         ik = i;
    //         break;
    ...
    // binary search on it
    long long int gt = n - k;
    long long int st = k - 1;
    ...
    // println("BAD:", sb, gb);
    // println("GUD:", st, gt);
    if (gb > st - su || sb > gt - gu){
        print(-1);
        // nline();
        continue;
    }

    print(sb + gb + abs(gb - sb));
    // nline();
}
nline();
