...
std::unordered_map<long long int, long long int> frequency;
...
std::vector<std::pair<long long int, long long int>> collapse;
for (auto& part: frequency) {
    collapse.push_back({part.second, part.first});
}

std::sort(collapse.begin(), collapse.end());

long long int presen = n;
long long int result = 0;

for (long long int i = 0; i < collapse.size(); i++) {
    auto current = collapse[i];
    
    if (i == 0 || current.first != collapse[i - 1].first) {
        
        long long int remaining = (long long int)collapse.size() - i;
        long long int diff = k - presen;
        
        if (diff % remaining == 0) {
            if (diff / remaining >= 1 - current.first) {
                result += 1;
            }
        }
    }

    presen -= current.first;
}
