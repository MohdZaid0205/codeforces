...
std::vector<long long int> H = std::vector<long long int>(n);
for (long long int i = 0; i < n; i++){ std::cin >> H[i]; }

std::vector<long long int> P = std::vector<long long int>(n);
for (long long int i = 0; i < n; i++){ std::cin >> P[i]; }

std::vector<std::pair<long long int, long long int>> HP;
...
long long int v = 0LL;
while (k > 0) {
    auto j = std::distance(H.begin(), std::upper_bound(H.begin(), H.end(), v + k));
    // long long int d = P[j];
...
//     long long int dh = H[i] - v;

//     long long int l = 1LL;
//     long long int r = (P[i] == 0) ? 2e9 : (k / P[i] + 1);
//     long long int ans = r;

//     while (l <= r) {
//         long long int m = l + (r - l) / 2;
//         long long int c = m * k - m * (m - 1) / 2 * P[i];
...
//     v += (long long)(ans * k - ans * (ans - 1) / 2 * P[i]);
//     k -= ans * P[i];
