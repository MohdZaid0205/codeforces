...
std::vector<std::vector<long long int>> array 
= std::vector<std::vector<long long int>>(n);
...
std::unordered_map<long long int, bool, custom_hash> chosen;
std::unordered_map<long long int, bool, custom_hash> seen;

for (int i = 0; i < n; i++) {
    std::vector<long long int> filtered;
    
    for (int j = array[i].size() - 1; j >= 0; j--) {
        long long int val = array[i][j];
        if (!seen[val]) { 
    ...
    for (long long int val : filtered) {
        seen[val] = false;
    }
...
auto compare = [](const std::vector<long long int>& A, const std::vector<long long int>& B, const std::unordered_map<long long int, bool, custom_hash>& chosen_map) {
    int iA = 0;
    int iB = 0;
    
    auto is_chosen = [&](long long int val) {
        auto it = chosen_map.find(val);
        return it != chosen_map.end() && it->second;
    };

    while (iA < A.size() && iB < B.size()) {
        while (iA < A.size() && is_chosen(A[iA])) iA++;
        while (iB < B.size() && is_chosen(B[iB])) iB++;
...
for (long long int i = 0; i < n; i++){
    ...
    for (long long int j = 0; j < n; j++){
        ...
        if (smallest_idx == -1 || compare(array[j], array[smallest_idx], chosen)) {
            smallest_idx = j;
        }
    }
    
    used[smallest_idx] = true;
    auto& smallest = array[smallest_idx];
    
    for (int k = 0; k < smallest.size(); k++) {
        if (!chosen[smallest[k]]) { 
            chosen[smallest[k]] = true;
            print(smallest[k]);
        }
    }
}
