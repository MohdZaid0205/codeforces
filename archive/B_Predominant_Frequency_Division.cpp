...
std::vector<long long int> part1 = {0};
for (long long int i = 0; i < n; i++) {
    part1.push_back(part1[i] + (arr[i] == 1 ? 1 : 0 ));
}

std::vector<long long int> part2 = {0};
for (long long int i = 0; i < n; i++) {
    part2.push_back(part2[i] + (arr[i] == 2 ? 1 : 0 ));
}

std::vector<long long int> part3 = {0};
for (long long int i = 0; i < n; i++) {
    part3.push_back(part3[i] + (arr[i] == 3 ? 1 : 0 ));
}

std::vector<long long int> parted(n + 1);
long long int pmin = INT32_MAX;

for (long long int i = n - 1; i >= 2; i--) {
    long long int val_i = 2 * part3[i] - i;
    pmin = std::min(pmin, val_i);
    parted[i] = pmin;
}

bool possible = false;
for (long long int j = 1; j <= n - 2; j++) {
    if ((part1[j] * 2 >= j) && (parted[j + 1] <= 2 * part3[j] - j)) {
        possible = true;
        break;
    }
}
