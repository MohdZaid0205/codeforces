...
// check for presence of larget prime before smaller prime
// or find a number that has multiple primes but alice can choose smaller
// and put it after larger.

auto primeFactors = [](long long int number) {
...
for (auto& e: arr){
    auto pf = primeFactors(e);
    long long int pmin = e;
    for (auto& f:pf){
        pmax = vmax(pmax, f);
        pmin = vmin(pmin, f);
    }
