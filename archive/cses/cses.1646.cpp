...
std::vector<std::vector<long long int>> sparse
    = std::vector<std::vector<long long int>>(
        (int)log2(n) + 1, std::vector<long long int>(n, INT64_MAX)
    );

sparse[0] = arr;

// populate sparsse indecies
for (long long int i = 1; i < (int)log2(n) + 1; i++){
    for (long long int j = 0; j + (1<<i) - 1 < n; j++){
        sparse[i][j] = sparse[i-1][j] + sparse[i-1][j+ (1<<(i-1))];
    }
}

// for (long long int i = 0; i < (int)log2(n) + 1; i++){
//     for (long long int j = 0; j + (1<<i) - 1 < n; j++){
//         print(sparse[i][j]);
//     }
//     nline();
