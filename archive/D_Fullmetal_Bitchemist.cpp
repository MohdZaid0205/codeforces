...
std::pair<long long int, long long int> recusive(
    std::string &s,
    std::vector<long long int> &zdp,
    std::vector<long long int> &odp,
    long long int i
) {
    if (i >= s.size()){
        return {0, 0};
    ...
    // consider incomings
    auto incoming = recusive(s, zdp, odp, i+1);
    if (s[i] == '0') {
        zdp[i] = incoming.first*2 + 1;
        odp[i] = incoming.second + 1;
    } else {
        zdp[i] = incoming.first + 1;
        odp[i] = incoming.second*2 + 1;
    }
...
std::vector<long long int> zdp = std::vector<long long int>(n, -1);
std::vector<long long int> odp = std::vector<long long int>(n, -1);

std::pair<long long int, long long int> result
        = recusive(s, zdp, odp, 1);
