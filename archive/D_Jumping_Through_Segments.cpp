...
std::vector<std::pair<long long int, long long int>> arr 
    = std::vector<std::pair<long long int, long long int>>(n);
...
long long int ll = INT64_MAX;
long long int rl = INT64_MAX;

for (long long int i = 0; i < n; i++){
    // println(arr[i].first, arr[i].second);
    // ll = vmin(ll, arr[i].first/(i+1) + (arr[i].first%(i+1) == 0)? 0LL: 1LL);
    // rl = vmin(rl, arr[i].second/(i+1) + (arr[i].second%(i+1) == 0)? 0LL: 1LL);
    // println(i, ll, rl);
}
