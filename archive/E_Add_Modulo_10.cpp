...
bool consistant_a = true;
long long int v_a = NONE;

bool consistant_b = true;
long long int v_b = NONE;

for (long long int i = 0; i < n; i++){
    long long int d = arr[i]%10;
    if (d == 0 || d == 5) {
        TYPE_B = true;
        long long int b = d == 5 ? 1 : 0;
        if (v_b == NONE)
            v_b = arr[i]/10 + b;
        else {
            consistant_b &= ((arr[i]/10 + b) == v_b);
        ...
        if (d == 6 || d == 3) {
            if (v_a == NONE) v_a = arr[i]/10;
            consistant_a &= std::abs(v_a - arr[i]/10) % 2 == 0;
        } else
        if (d == 7 || d == 9) {
            if (v_a == NONE) v_a = arr[i]/10 + 2;
            consistant_a &= std::abs(v_a - arr[i]/10 - 2) % 2 == 0;
            
        } else {
            if (v_a == NONE) v_a = arr[i]/10 + 1;
            consistant_a &= std::abs(v_a - arr[i]/10 - 1) % 2 == 0;
        }
    }
}

// println(TYPE_A, TYPE_B);
// println(consistant_a, consistant_b);
// println();

if ((TYPE_A & TYPE_B) || (TYPE_A & !consistant_a) || (TYPE_B & !consistant_b))
    println("No");
else
    println("Yes");
