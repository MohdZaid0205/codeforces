...
std::vector<long long int> zro = std::vector<long long int>(n);
std::vector<long long int> dgt = std::vector<long long int>(n);
for (long long int i = 0; i < n; i++){
    ...
    zro[i] = cnt;
    dgt[i] = see;
    // println("n:", arr[i], cnt, see);
}

long long int size = std::accumulate(dgt.begin(), dgt.end(), 0);
std::sort(zro.begin(), zro.end(), std::greater<long long int>());
