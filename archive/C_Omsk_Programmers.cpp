...
std::vector<long long int> nrr;
std::vector<long long int> mrr;
...
for (long long int i = 0; i < nrr.size(); ++i) {
    for (long long int j = 0; j < mrr.size(); ++j) {
        result = vmin(result, i + j + abs(nrr[i] - mrr[j]));
    }
}
