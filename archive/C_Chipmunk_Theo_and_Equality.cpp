...
std::unordered_set<long long int> vals;
bool initialized = false;

for (auto& a: arr){
    std::unordered_set<long long int> achives;
    while (a != 1) {
        achives.insert(a);
        if (a%2 == 0){
            a/=2;
    ...
    if (!initialized){
        ...
        std::unordered_set<long long int> intersection;
        const auto& smaller = (vals.size() < achives.size()) ? vals : achives;
        const auto& larger  = (vals.size() < achives.size()) ? achives : vals;
        for (const int& num : smaller) {
            if (larger.find(num) != larger.end()) { 
...
long long int result = INT64_MAX;

for (auto& achive2: vals){
    ...
    result = vmin(result, result2);
    // println(vmin(result, result2), achive2, achive2, result, result2);
}
