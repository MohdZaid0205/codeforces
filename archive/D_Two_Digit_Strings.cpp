...
std::vector<int> al(len_a);
std::vector<int> bl(len_b);
...
// Using 0 as the default value instead of -1 simplifies the max() logic below
std::vector<std::vector<int>> dp(len_b, std::vector<int>(len_a, 0));

for (int j = 0; j < len_b; j++) {
    for (int i = 0; i < len_a; i++) {
        if (al[i] == bl[j]) {
            int dd = (i > 0 && j > 0) ? dp[j - 1][i - 1] : 0;
            dp[j][i] = dd + 1;
        } else {
            int u = (j > 0) ? dp[j - 1][i] : 0;
            int l = (i > 0) ? dp[j][i - 1] : 0;
            dp[j][i] = std::max(u, l);
        }
    }
}
