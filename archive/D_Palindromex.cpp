...
std::vector<long long int> arr = std::vector<long long int>(2*n);
for (long long int i = 0; i < 2*n; i++){ std::cin >> arr[i]; }
...
long long int max_mex = 1;

// i feel like this is stupid
// BUT wither plaindrome contains both 0s or both 1s
std::vector<long long int> found_u0;
bool made_palindrome_using_both_0s = true;


long long int l = value_index[0][0];
long long int r = value_index[0][1];

// come towards middle
while (l <= r) {
    found_u0.push_back(arr[l]);
    if (arr[l] != arr[r]){
        made_palindrome_using_both_0s = false;
        break;
    }
    l ++;
    r --;
...
if (made_palindrome_using_both_0s){
    // go towards right till you can
    l = value_index[0][0];
    r = value_index[0][1];
    ...
    std::sort(found_u0.begin(), found_u0.end());
    long long int mex = 0;
    // print("USING 0:");
    for (auto e: found_u0){
        if (e == mex){
    ...
    // nline();
    max_mex = vmax(max_mex, mex);
}

std::vector<long long int> found_u1;
bool made_palindrome_using_both_1s = true;
...
// come towards middle
while (l <= r) {
    found_u1.push_back(arr[l]);
    if (arr[l] != arr[r]){
        made_palindrome_using_both_1s = false;
        break;
    }
    l ++;
    r --;
...
if (made_palindrome_using_both_1s){
    // go towards right till you can
    l = value_index[1][0];
    r = value_index[1][1];
    ...
    std::sort(found_u1.begin(), found_u1.end());
    long long int mex = 0;
    // print("USING 1:");
    for (auto e: found_u1){
        if (e == mex){
