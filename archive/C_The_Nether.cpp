...
std::vector<std::vector<long long int>> expected 
    = std::vector<std::vector<long long int>>(n+1);
for (long long int i = 1; i <= n; i++){
    ...
    highest = vmax(highest, k);
    expected[k].push_back(i);
}

// select the first node
// we know highest length is non empty because it occured
std::vector<long long int> result;
result.push_back(expected[highest][0]);
highest --;
for (long long int i = highest; i > 0; i--){
    for (auto& v:expected[i]){
        print("?", result[0], result.size() + 1, v);
        for (auto& u:result){
            print(u);
        }
        nline();
