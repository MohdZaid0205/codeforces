...
for (long long int i = 0; i < n; i++) {
    ...
    long long int commfactor = gcd(std::abs(arr[i]), std::abs(brr[i]));
    
    long long int b_norm = brr[i] / commfactor;
    long long int a_norm = arr[i] / commfactor;
    ...
    fraction[{b_norm, a_norm}] += 1;
}
    

long long int result = 0;
for (auto& f: fraction){
    result = vmax(result, f.second);
}
