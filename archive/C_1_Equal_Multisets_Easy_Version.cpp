...
std::vector<long long int> brr = std::vector<long long int>(n);

for (long long int i = 0; i < n; i++){ std::cin >> arr[i]; }
for (long long int i = 0; i < n; i++){ std::cin >> brr[i]; }

// check for consistency (no repeated values in brr)
std::unordered_map<long long int, int> lookup_b 
                = std::unordered_map<long long int, int>();
std::unordered_map<long long int, int> lookup_a 
                = std::unordered_map<long long int, int>();
// populate lookup table with brr enteries
...
for (long long int i = 0; i < n; i++){
    ...
    if (brr[i] == -1)
        continue;
    // inconsistency with permutation constraint
    if (lookup_b.find(brr[i]) != lookup_b.end()){
        possible = false;
    }
    lookup_b[brr[i]] = i;
}
...
// for (long long int i = 0; i < n; i++){
//     long long int l = i < k ? i : k;
//     long long int r = n - i - 1 < k
//                         ? n - i - 1 : k;
//     long long int lim_l = i - l + 1;
//     long long int lim_r = i + r - 1;
//     println(arr[i], ":" , l , r, lim_l, lim_r);
// }
...
if (r <= l) { 
    ...
    for (long long int i = l; i < r; i++){
        if (brr[i] != -1){
            if (lookup_a.find(brr[i]) == lookup_a.end()){
                possible = false;
                // println("lookup:", brr[i], l, r, "not found");
            } else {
                // println("lookup:", brr[i], l, r, lookup_a[brr[i]]);
                if (lookup_a[brr[i]] < l 
                || lookup_a[brr[i]] >= r){
                    possible = false;
                }
