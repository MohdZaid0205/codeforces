...
for (long long int i = 0; i < log2l(n) + 1; i ++) {
    sparse.push_back(std::vector<long long int>(n));
}
...
for (long long int i = 1; i < log2l(n) + 1; i ++) {
    for (long long int j = 0; j < n; j ++) {
        sparse[i][j] = gcd(
            sparse[i-1][j], 
            sparse[i-1][(j + (1 << (i - 1)))%n]
        );
    }
}

// for (auto& level:sparse) {
...
auto querry = [&](long long int L, long long int R) {
    long long int stride = log2l(R - L + 1);
    return gcd(
        sparse[stride][L],
        sparse[stride][(R - (1 << stride) + 1)%n]        
    );
};
...
while (l <= h) {
    ...
    bool can = true;
    long long int target = querry(0, m-1);
    // println("size:", m);
    for (long long int i = 1; i < n; i++){
        long long int found = querry(i, i + m - 1);
        // println("target:", target, "found:", found, "l:", i, "r:", i + m - 1);
        if (found != target){
            can = false;
            break;
        }
