...
long long int rb = r2/l1;
long long int lb = l2/r1;
long long int t = 1;
...
while (t <= rb){
    long long int lx = (l2 + t - 1)/t;
    long long int rx = r2/t;
    long long int ll = vmax(lx, l1);
    long long int rr = vmin(rx, r1);
    // println(">>", t, ll, rr, lx, rx, l1, r1);
    if (ll <= rr)
        result += rr - ll + 1;
    t *= k;
}
