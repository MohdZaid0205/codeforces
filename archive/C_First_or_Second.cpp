...
// long long int recurse(std::vector<std::vector<long long int>>& dp, std::vector<std::vector<bool>>& fl, std::vector<long long int> arr, long long int i, long long int j){
//     if (i >= arr.size() || j >= arr.size()) return 0LL;
//     if (fl[i][j]) return dp[i][j];

//     fl[i][j] = true;
//     long long int f = recurse(dp, fl, arr, j, j+1) + arr[i];
//     long long int s = recurse(dp, fl, arr, i, j+1) - arr[j];
//     dp[i][j] = vmax(f, s);
//     return dp[i][j];
// }
...
//     = std::vector<std::vector<long long int>>(n, std::vector<long long int>(n, 0));
// std::vector<std::vector<bool>> fl
//     = std::vector<std::vector<bool>>(n, std::vector<bool>(n, false));
// long long int X = recurse(dp, fl, arr, 0, 1);
// println(X);
...
for (int i = n - 2; i >= 1; i--) {
    long long X = arr[i] + vmax(0LL, arr[i] + M);
    M = vmax(M, X);
}

println(-s + vmax(0LL, arr[0] + M));
