...
for (long long int i = 0; i < n; i++){ brr[i]  = arr[i]; }

std::sort(brr.begin(), brr.end());
long long int mi = brr[ 0 ];
long long int mx = brr[n-1];
long long int rr = INT64_MAX;
bool sorted = true;

for (long long int i = 0; i < n; i++){
    // println(arr[i], brr[i], mi, mx);
    if (arr[i] != brr[i]){
        sorted = false;
        rr = vmin(rr, vmax(arr[i] - mi, mx - arr[i]));
    }
}
