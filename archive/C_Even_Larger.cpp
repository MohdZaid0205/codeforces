...
for (long long int i = 0; i < n; i+=2){
    long long int l = INT64_MAX;
    long long int r = INT64_MAX;
    if (i != 0){
        l = arr[i-1];
    }
    if (i != n-1){
    ...
    result += vmax(0LL, arr[i] - vmin(l, r));
    arr[i] = vmin(arr[i], l, r);
}
...
for (long long int i = 1; i < n; i+=2){
    ...
    if (i != n-1){
        long long int s = vmax(0LL, l + r - arr[i]);
        result += s;
        arr[i+1] = vmax(arr[i+1] - s, 0LL);
    }
}
