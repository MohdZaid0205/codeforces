...
while (split + power*9*powl(10LL, power - 1) < n){
    split += power*9*powl(10LL, power - 1);
    power ++;
}


long long int trash = (n - split)/power + (n - split)%power;
// long long int remain = (n - split)%power;
long long int result = 0;
long long int digit = power;
long long int highest = pow(10LL, power-1);

// println(highest, digit);

while (digit >= 0){
    long long int p = powl(10LL, digit);
    long long int d = (split + trash)/p;
    // long long int k = pow(10LL, digit+1);
    d = d%10;
    println("place:", digit, d);
    for (long long int i = 0; i < 10; i++){
        println("   >occurence", i, "=", i*highest);
        result += d*highest;
    }
    println("   >>trucated", d, "=", ((split+trash)%p)*highest);
    result += ((split+trash)%p)*d;
    digit --;
}

println(split, power - 1, split+trash);
// nline();
