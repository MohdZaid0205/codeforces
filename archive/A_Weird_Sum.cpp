...
std::vector<std::vector<long long int>> arr(n, std::vector<long long int>(m, 0));
...
auto process = [](std::vector<long long int>& vec, long long int l) {
    // std::vector<long long int> pref(l + 1, 0);
    // std::vector<long long int> post(l + 1, 0);
    
    // for (long long int i = 0; i < l; i++) {
    //     pref[i + 1] = pref[i] + vec[i];
    //     post[l-i-1] = post[l-i] + vec[l-i-1];
    // }
    ...
    for (long long int i = 0; i < l; i++) {
        res += vec[i] * i - vec[i] * (l - 1 - i);
    }
    
    return res;
};

// auto reconstruct = [](std::vector<long long int>& vec, long long int l) {
//     std::vector<long long int> rec(l, 0);
//     for (auto& e:vec) {
//         rec[e] += 1;
//     }
//     return rec;
...
std::unordered_map<long long int, std::vector<long long int>> collections;
std::unordered_map<long long int, std::vector<long long int>> reflections;

for (long long int i = 0; i < n; i++) {
    for (long long int j = 0; j < m; j++) {

        // if (collections.find(arr[i][j]) == collections.end())
        //     collections[arr[i][j]] = std::vector<long long int>();
        collections[arr[i][j]].push_back(j);

        // if (reflections.find(arr[i][j]) == reflections.end())
        //     reflections[arr[i][j]] = std::vector<long long int>();
        reflections[arr[i][j]].push_back(i);
    }
}
...
for (auto& item:collections) {
    // println(item.first, "TOP");
    // std::vector<long long int> rec = reconstruct(item.second, m);
    std::sort(item.second.begin(), item.second.end());
    long long int ans = process(item.second, item.second.size());
    res += ans;
}

for (auto& item:reflections) {
    // println(item.first, "REF");
    // std::vector<long long int> rec = reconstruct(item.second, n);
    std::sort(item.second.begin(), item.second.end());
    long long int ans = process(item.second, item.second.size());
    res += ans;
}
