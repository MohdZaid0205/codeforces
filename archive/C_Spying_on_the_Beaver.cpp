...
std::vector<long long int> arr = std::vector<long long int>(n-1);
for (long long int i = 0; i < n-1; i++){ std::cin >> arr[i]; }

long long int m;
std::cin >> m;

std::set<long long int> brr;
...
// std::unordered_map<long long int, std::vector<long long int>> adj;
