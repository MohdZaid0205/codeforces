...
std::deque<long long int> req = {1, 5, 4, 3};
long long int r = 0;

for (long long int i = 0; i < vmin(m, n)/2; i ++) {
    std::deque<long long int> val = {0, 0, 0, 0};

    for (long long int j = i; j < m - i; j++){
    ...
    for (long long int j = i + 1; j < n - i - 1; j ++) {
        val.push_back(arr[j][m - i - 1]);
        val.pop_front();
        if (val == req){
            r += 1;
        }
    ...
    for (long long int j = m - i - 1; j >= i; j--){
        val.push_back(arr[n - i - 1][j]);
        val.pop_front();
        if (val == req){
            r += 1;
        }
    ...
    for (long long int j = n - i - 2; j > i; j --) {
